...
class Table(object):
    def config_db(self, pkg):
        '''banche: anagrafica banche'''

        tbl = pkg.table('banche', pkey='id',
                        pkey_columns='sog__cod,cod',
                        name_long='!![it]Banca',
                        name_plural='!![it]Banche',
        ...
        tbl.column('iban', dtype='A', size=':32', 
                   name_long='!![it]IBAN',
                   )
        ...
        tbl.formulaColumn('caption', "'B-'||$cod||' - '||$desc",
                          name_long='!![it]Titolo')
    ...
    def pkeyValue(self, record):
        ...
        if not record['cod']:
            q = self.db.table('anag.banche').query(
