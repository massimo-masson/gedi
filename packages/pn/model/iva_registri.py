...
class Table(object):
    def config_db(self, pkg):
        '''iva_registri: Registri IVA per soggetto'''

        tbl = pkg.table('iva_registri',
                        pkey='cpk',
        ...
        # CPK
        #
        cpk = tbl.compositeColumn('cpk', columns='sog__cod,cod',
                                  name_long = '!![it]Registro IVA',
                                  )

        # foreign key to attivita' IVA
        iva_attivita__cod = tbl.column('iva_attivita__cod', dtype = 'A', size = ':32',
                                       name_long = '!![it]Attività IVA',
                                       #unmodifiable=True,
                                       #validate_notnull = True
                                       )
        # iva_attivita__cod.relation('pn.iva_attivita.cod', mode = 'foreignkey',
        #                            relation_name = 'attivita_iva', 
        #                            one_one=True,
        #                            onDelete = 'raise'
        #                            )

        ivaattivita__fk = tbl.compositeColumn('iva_attivita__fk', 'sog__cod,iva_attivita__cod',
                                              name_long = '!![it]Attività IVA'
                                              )
        ivaattivita__fk.relation('pn.iva_attivita.cpk', mode = 'foreignkey',
                                 relation_name = 'attivita_iva_fk',
                                 one_one = True,
                                 onDelete = 'raise',
                                 )
