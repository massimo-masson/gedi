...
class Table(object):
    def config_db(self, pkg):
        ...
        esercizi__cod = tbl.column('esercizi__cod', dtype = 'A', size = ':32',
                                  name_long = '!![it]Esercizio',
                                  validate_notnull = True
                                  )
        # esercizi__id.relation('pn.sog_esercizi.id', mode = 'foreignkey',
        #                       relation_name = 'registrazioni_esercizio', 
        #                       onDelete = 'raise'
        #                       )

        esercizi__fk = tbl.compositeColumn('esercizi__fk', columns='sog__cod,esercizi__cod',
                                           name_long = '!![it]Esercizio',
                                           )
        esercizi__fk.relation('pn.sog_esercizi.cpk', mode = 'foreignkey',
                              relation_name = 'registrazioni_esercizio',
                              onDelete = 'raise',
                              )

        # foreign key to rc_grp - gruppo di registrazione di questa rilevazione
        ...
        # foreign key to iva_registri - eventuale registro iva
        iva_registri__cod = tbl.column('iva_registri__cod', 
                                       dtype = 'A', size = ':32',
                                        name_long = '!![it]Registro IVA',
                                        #validate_notnull = True
                                        )
        # iva_registri__id.relation('pn.iva_registri.id', mode = 'foreignkey',
        #                          relation_name = 'registrazioni_ivaregistro', 
        #                          onDelete = 'raise'
        #                          )

        iva_registri__fk = tbl.compositeColumn('iva_registri__fk', 'sog__cod,iva_registri__cod',
                                               name_long = '!![it]Registro IVA',
                                               )
        iva_registri__fk.relation('pn.iva_registri.cpk',
                                  mode = 'foreignkey',
                                  relation_name = 'registrazioni_ivaregistro',
                                  onDelete = 'raise'
                                  )

        # foreign key to divisioni - eventuale divisione
        ...
        tbl.formulaColumn('tot_cda_dare_udc', dtype='N', 
                          ...
                          select=dict(table='pn.rc_rcg_cda',
                                      columns='COALESCE(SUM($dare_udc), 0)',
                                      where='@rc_rcg__id.rc__id=#THIS.id'
                                      )
                          )
        ...
        tbl.formulaColumn('numero_cda_movimentati', dtype='N', 
                          ...
                          select=dict(table='pn.rc_rcg_cda',
                                      columns='COUNT(DISTINCT $cda_centri__cod)',
                                      where='@rc_rcg__id.rc__id=#THIS.id'
                                      )
                          )
        ...
        tbl.formulaColumn('tot_com_dare_udc', dtype='N', 
                          ...
                          select=dict(table='pn.rc_rcg_com',
                                      columns='COALESCE(SUM($dare_udc), 0)',
                                      where='@rc_rcg__id.rc__id=#THIS.id'
                                      )
                          )
        ...
        tbl.formulaColumn('numero_com_movimentate', dtype='N', 
                          ...
                          select=dict(table='pn.rc_rcg_com',
                                      columns='COUNT(DISTINCT $commesse__id)',
                                      where='@rc_rcg__id.rc__id=#THIS.id'
                                      )
                          )
